...
PCM was benchmarked against leading agent memory systems (**Zep / Graphiti**, **Mem0**) and standard information retrieval baselines (**Hybrid RAG**, **Naive RAG**):

1. **Peripheral Cognitive Mesh (PCM)**: Dual-loop architecture with continuous Ebbinghaus decay, pinned guardrails, associative spreading activation, and PAE slotted context.
2. **Temporal Knowledge Graph (Zep / Graphiti)**: Bi-temporal entity-relation knowledge graph with episode, entity, and community subgraphs.
3. **Fact-Extraction Vector (Mem0)**: Atomic fact extraction via write-time LLM prompting, stored in dense vector embeddings with flat bulleted recall.
4. **Hybrid Retrieval**: BM25 sparse keyword search + dense vector + unconditional cross-encoder rerank.
5. **Naive RAG**: Dense vector retrieval (top-5 cosine similarity dump).

### 5.1 The Comparative Benchmark Matrix

The following results are generated by the automated comparative evaluation suite (`bun run benchmark`):

| Architecture | Hit Rate @ 1 | Hit Rate @ 3 | MRR | Avg Tokens/Turn | Recall Latency (p50) | Ingest Latency (p50) | Temporal Contradiction |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Peripheral Cognitive Mesh (PCM)** | **100.0%** | **100.0%** | **1.000** | **93 tokens** | **< 30ms** | **< 2ms** | **✅ Resolved (100%)** |
| **Temporal Graph (Zep / Graphiti)** | 66.7% | 83.3% | 0.783 | 145 tokens | 155ms – 250ms | 800ms – 1,500ms | ✅ Resolved |
...
| **Naive RAG (Vector Dump)** | 16.7% | 83.3% | 0.478 | 325 tokens | 35ms – 60ms | 20ms – 40ms | ❌ Failed (Amnesia) |

### 5.2 Key Comparative Insights

#### 1. Architectural Reversal & Temporal Contradiction
- **Setup**: Ingest architectural decision $D_1$ at $t = -90$ days (*"Use Vitest"*). Ingest superseding pinned decision $D_2$ (*"Always use Bun test runner; do not install vitest"*). Query: *"Which test framework should we run?"*
- **Results**:
  - **Mem0**: Fails without an explicit write-time LLM `DELETE` instruction; legacy Vitest and Bun test compete purely on vector similarity, resulting in contradiction amnesia.
  - **Zep (Graphiti)**: Resolves temporal order if bi-temporal timestamps are explicitly recorded, but behavioral rules get fragmented into entity nodes.
  - **PCM: 100% Top-1 Resolution**. $D_1$ decays naturally via Ebbinghaus dynamics ($S \to 0$), while $D_2$ is pinned ($S = 1.0$), ensuring decisive separation.

#### 2. Prompt Footprint & Context Economy
- Mem0 injects 195 tokens of flat bulleted facts; Zep injects 145–450 tokens of entity subgraphs and community summaries.
- PCM formats into structured PAE slots (`[ASKER CONTEXT]`, `[SITUATIONAL CONTEXT]`) at **93 tokens**—a **36% to 71% reduction** in context clutter, mitigating "Lost in the Middle" attention degradation.

#### 3. Hot-Path Write & Recall Latency
- Ingesting memories into Mem0 and Zep incurs heavy LLM parsing or entity synthesis pipelines (**800ms to 2,500ms** per write).
- PCM writes in **< 2ms** on exact matches and **~20ms** on vectorized batches.
- On recall, PCM responds in **< 30ms** (and **< 1ms** on Pinned Cache hits), compared to **155ms–250ms** for Zep and **600ms+** for Mem0ᵍ graph traversals.
